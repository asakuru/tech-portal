...
if (isset($_POST['save_user'])) {
    ...
    try {
        ...
        if ($target_id == $_SESSION['user_id']) { $_SESSION['username'] = $new_name; }
...
$start_ts = 0; $end_ts = 0; $date_title = "";

if ($view === 'daily') {
    $start_ts = strtotime($selected_date); $end_ts = $start_ts;
    ...
    $ts = strtotime($selected_date);
    $start_ts = strtotime(date('Y-m-01', $ts)); $end_ts = strtotime(date('Y-m-t', $ts));
...
foreach ($display_jobs as $job) {
    ...
    if (($job['extra_per_diem'] ?? 'No') === 'Yes') $stats['extra_pd_total'] += $rates['per_diem'];
    if ($job['install_type'] !== 'DO') $unique_worked_dates[$job['install_date']] = true;
}
...
while ($curr <= $end_ts) {
    ...
    $isExplicitDayOff = false; $dayJobPay = 0; $dayExtraPD = 0;
    foreach ($display_jobs as $dj) {
        if ($dj['install_date'] === $loopDate) {
            if ($dj['install_type'] === 'DO') $isExplicitDayOff = true;
            $dayJobPay += (float)$dj['pay_amount'];
            if (($dj['extra_per_diem'] ?? 'No') === 'Yes') $dayExtraPD += $rates['per_diem'];
        }
    }
    
    $hasWork = isset($unique_worked_dates[$loopDate]);
    $dailyBase = 0;
    if ($dayOfWeek == 0) { $dailyBase = $rates['per_diem']; } 
    else { if ($hasWork && !$isExplicitDayOff) { $dailyBase = $rates['per_diem']; } }
...
if ($view === 'weekly' && !empty($display_jobs)) { $stats['lead_pay'] = $lead_pay_amount; }
...
function renderRow($job, $install_names, $rates, $index) {
    ...
    $visualCodes = [];
    if (!empty($job['install_type'])) { $p = $rates[$job['install_type']] ?? 0; $visualCodes[] = "<b>{$job['install_type']}</b> $" . number_format($p, 2); }
    if (($job['spans']??0) > 0) $visualCodes[] = "Spans({$job['spans']})";
    if (($job['conduit_ft']??0) > 0) $visualCodes[] = "Cond({$job['conduit_ft']}')";
    if (($job['jacks_installed']??0) > 1) $visualCodes[] = "Jacks({$job['jacks_installed']})";
    if (($job['extra_per_diem'] ?? 'No') === 'Yes') $visualCodes[] = "<span style='color:var(--success-text); font-weight:bold;'>+Extra PD</span>";
...
<head>
    ...
    <style>
        ...
        .selector { padding: 8px; border-radius: 6px; border: 1px solid var(--border); background: var(--bg-input); color: var(--text-main); font-size: 0.9rem; }
        
        .admin-cal-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 5px; margin-bottom: 20px; }
        .admin-cal-head { text-align: center; font-weight: bold; font-size: 0.8rem; color: var(--text-muted); padding-bottom: 5px; }
        .admin-cal-day { background: var(--bg-card); border: 1px solid var(--border); border-radius: 6px; min-height: 60px; padding: 4px; display: flex; flex-direction: column; justify-content: space-between; }
        .admin-cal-day.empty { background: transparent; border: none; }
        
        .user-list-row {
            display: grid; grid-template-columns: 1fr 80px 1.5fr; gap: 10px; align-items: center;
            padding: 10px; border-bottom: 1px solid var(--border);
        }
        .user-list-row:last-child { border-bottom: none; }
        @media (max-width: 600px) {
            .user-list-row { grid-template-columns: 1fr; gap: 5px; background: var(--bg-input); margin-bottom: 10px; border-radius: 8px; border: 1px solid var(--border); }
        }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <div class="stats-grid">
        <div class="stat-box"><div class="stat-label">Jobs</div><div class="stat-val">$<?= number_format($stats['job_total'], 2) ?></div></div>
        <div class="stat-box"><div class="stat-label">Base PD</div><div class="stat-val">$<?= number_format($stats['base_pd_total'], 2) ?></div></div>
        <?php if ($view !== 'daily'): ?>
        <div class="stat-box" style="border-color:var(--accent);"><div class="stat-label" style="color:var(--accent);">Lead Pay</div><div class="stat-val" style="color:var(--accent);">$<?= number_format($stats['lead_pay'], 2) ?></div></div>
        <?php endif; ?>
        <div class="stat-box" style="background:var(--success-bg); border-color:var(--success-text);"><div class="stat-label" style="color:var(--success-text);">Total</div><div class="stat-val" style="color:var(--success-text);">$<?= number_format($stats['grand_total'], 2) ?></div></div>
    </div>

    <?php if($view === 'monthly'): ?>
    <div class="box" style="padding:10px; overflow:hidden;">
        <div class="admin-cal-grid">
            <div class="admin-cal-head">M</div><div class="admin-cal-head">T</div><div class="admin-cal-head">W</div><div class="admin-cal-head">T</div><div class="admin-cal-head">F</div><div class="admin-cal-head">S</div><div class="admin-cal-head">S</div>
            <?php
            $first_of_month = date('Y-m-01', strtotime($selected_date)); $day_of_week = date('N', strtotime($first_of_month)); $days_in_month = date('t', strtotime($selected_date));
            for($x=1; $x<$day_of_week; $x++) echo "<div class='admin-cal-day empty'></div>";
            for($day=1; $day<=$days_in_month; $day++) {
                $info = $calendar_days[$day] ?? ['total'=>0, 'is_off'=>false, 'has_work'=>false, 'base'=>0];
                $badges = ""; if($info['is_off']) $badges .= "🚫"; elseif($info['has_work']) $badges .= "🔵";
    ...
    <?php if($view === 'daily' && isset($is_closed) && $is_closed): ?>
        <div style="margin-bottom:20px; text-align:center;">
            ...
            <button style="background:#f59e0b; padding:10px 20px; border:none; border-radius:6px; cursor:pointer; font-weight:bold;">Reopen This Day</button></form>
        </div>
    <?php endif; ?>
    ...
    <div class="box" style="padding:0; overflow:hidden;">
            ...
            <table id="jobsTable" style="width:100%; border-collapse:collapse;">
                <thead>
                    <tr style="background:var(--bg-input); text-align:left;">
                        <th style="padding:10px;">Date</th><th style="padding:10px;">Job</th><th style="padding:10px;">Codes</th><th style="padding:10px; text-align:right;">Pay & Action</th>
                    </tr>
                </thead>
                <tbody>
        ...
        <?php if(empty($display_jobs)): ?><p style="text-align:center; color:var(--text-muted); padding:20px;">No jobs found.</p><?php endif; ?>
    </div>
    ...
    <div class="box" style="margin-top:30px; border-top: 4px solid var(--primary);">
        ...
        <form method="post" style="background:var(--bg-input); padding:15px; border-radius:8px; margin-bottom:20px; border:1px solid var(--border);">
            <div style="font-weight:bold; margin-bottom:10px; color:var(--primary);">Change My Password</div>
            <div style="display:flex; gap:10px; flex-wrap:wrap;">
                <input type="password" name="current_pass" placeholder="Current Password" required style="flex:1; padding:8px; border-radius:4px; border:1px solid var(--border);">
                <input type="password" name="new_pass" placeholder="New Password" required style="flex:1; padding:8px; border-radius:4px; border:1px solid var(--border);">
        ...
        <form method="post" style="background:var(--bg-input); padding:15px; border-radius:8px; margin-bottom:20px; border:1px solid var(--border);">
            ...
            <div style="display:flex; gap:10px; flex-wrap:wrap;">
                <input type="text" name="new_username" placeholder="Username" required style="flex:1; padding:8px; border-radius:4px; border:1px solid var(--border);">
                <input type="password" name="new_password" placeholder="Password" required style="flex:1; padding:8px; border-radius:4px; border:1px solid var(--border);">
                <select name="new_role" style="padding:8px; border-radius:4px; border:1px solid var(--border);"><option value="tech">Tech</option><option value="admin">Admin</option></select>
                <button type="submit" name="create_user" class="btn btn-small" style="background:var(--success-bg); color:var(--success-text);">Add User</button>
            </div>
        </form>

        <div style="border:1px solid var(--border); border-radius:8px; overflow:hidden;">
            <div class="user-list-row" style="background:var(--bg-input); font-weight:bold; border-bottom:2px solid var(--border);">
                <div>Username</div><div>Role</div><div>Action</div>
            </div>
            ...
            <form method="post" class="user-list-row">
                ...
                <div>
                    <span class="badge" style="background:<?= $u['role']=='admin'?'var(--primary)':'var(--text-muted)' ?>; color:white; padding:2px 6px; border-radius:4px; font-size:0.8rem;">
                        <?= strtoupper($u['role']) ?>
                    </span>
                </div>
                ...
                <div style="display:flex; gap:5px; align-items:center;">
                    ...
                    <button type="submit" name="delete_user" class="btn btn-small btn-danger" onclick="return confirm('Delete user <?= $u['username'] ?>?');" title="Delete User">❌</button>
                    <?php endif; ?>
                </div>
            </form>
            <?php endforeach; ?>
