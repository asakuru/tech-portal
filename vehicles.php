...
<div class="container">
    ...
    <?php if ($msg): ?>
        <div class="alert" style="border-left:4px solid var(--success-text); margin-bottom:20px;"><?= htmlspecialchars($msg) ?></div>
    <?php endif; ?>
    
    <?php if ($error): ?>
        <div class="alert" style="background:var(--danger-bg); color:var(--danger-text); margin-bottom:20px;"><?= htmlspecialchars($error) ?></div>
    <?php endif; ?>
    
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:25px;">
        <div>
            <h2 style="margin:0;">🚗 My Vehicles</h2>
            <p style="color:var(--text-muted); margin:5px 0 0;">Track maintenance, fuel, and total cost of ownership</p>
        </div>
            ...
            <?php foreach ($vehicles as $v): ?>
                <div class="vehicle-card <?= $v['is_primary_vehicle'] ? 'primary' : '' ?>">
                    <div class="vehicle-header">
                        <div>
                            <h3 class="vehicle-title"><?= htmlspecialchars($v['year'] . ' ' . $v['make'] . ' ' . $v['model']) ?></h3>
                    ...
                    <div class="vehicle-meta">
                        ...
                        <?php endif; ?>
                        <span>📍 <?= number_format($v['current_mileage']) ?> mi</span>
                    </div>
                    ...
                    <div class="vehicle-stats">
                        ...
                        <div class="stat-item">
                            <div class="stat-value"><?= $v['avg_mpg'] > 0 ? number_format($v['avg_mpg'], 1) : '--' ?></div>
                            <div class="stat-label">Avg MPG</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-value">$<?= number_format($v['total_service_cost'] + $v['total_fuel_cost']) ?></div>
                    ...
                    <div class="vehicle-actions">
                        ...
                        <a href="vehicle_edit.php?id=<?= $v['id'] ?>&tab=service" class="btn btn-small btn-secondary">🔧 Service</a>
                        <a href="vehicle_edit.php?id=<?= $v['id'] ?>&tab=fuel" class="btn btn-small btn-secondary">⛽ Fuel</a>
                        <a href="?delete=<?= $v['id'] ?>" onclick="return confirm('Delete this vehicle and all its records?')" class="btn btn-small btn-danger">🗑️</a>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
