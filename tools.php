...
// --- ROBUST ADMIN CHECK ---
// Matches logic in index.php and nav.php
...
<head>
    ...
    <style>
        ...
        .res-date { font-size: 0.85rem; color: var(--text-muted); font-weight: bold; margin-bottom: 2px; }
        .res-title { font-weight: bold; font-size: 1rem; color: var(--primary); }
        .res-sub { font-size: 0.9rem; color: var(--text-main); margin-top: 2px; }
        .res-addr { font-size: 0.8rem; color: var(--text-muted); margin-top: 4px; }
        
        .tag { 
            padding: 3px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: bold; 
            background: #eee; color: #555; display: inline-block; margin-left: 5px;
        }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <form method="get" class="search-box">
        <input type="text" name="q" class="search-input" placeholder="Search ticket #, name, address..." value="<?= htmlspecialchars($term) ?>" autocomplete="off" autofocus onfocus="this.select()">
    ...
    <?php if(!empty($term)): ?>
        ...
        <div class="results-list">
            <?php if(empty($results)): ?>
                <div style="text-align:center; padding:40px; color:var(--text-muted); background:var(--bg-input); border-radius:8px;">
            ...
            <?php else: ?>
                <?php foreach($results as $r): 
                    ...
                    if(trim($name) === '') $name = $r['cust_name']; // Fallback to old field
                    if(empty(trim($name))) $name = "Unknown Customer";
                    
                    // Handle Address (New fields vs Old fields)
                    $addr = $r['cust_street'] . ' ' . $r['cust_city'];
                    if(trim($addr) === '') $addr = $r['cust_address']; // Fallback to old field
                ?>
