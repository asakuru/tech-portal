...
// 3. Process Data
$month_data = [];
$total_work = 0; $total_std_pd = 0; $total_ext_pd = 0;

for ($d = 1; $d <= $days_in_month; $d++) {
    $date = sprintf("%s-%02d-%02d", $year, $month, $d);
    $month_data[$date] = [
        'date' => $date, 'day_num' => $d,
        'work' => 0.0, 'std_pd' => 0.0, 'ext_pd' => 0.0, 'total' => 0.0,
        'miles' => 0, 'fuel' => 0.0, 'net' => 0.0,
        'is_closed' => false,
        'jobs' => []
    ];
}
...
foreach ($month_data as $date => &$day) {
    ...
    foreach($day['jobs'] as $job) { if($job['type'] != 'DO') $has_eligible_work = true; }
...
<head>
    ...
    <style>
        .cal-wrapper { display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; background: var(--border); border: 1px solid var(--border); }
        .cal-header { background: var(--bg-input); text-align: center; padding: 10px 0; font-weight: bold; font-size: 0.9rem; }
        .cal-day { background: var(--bg-card); min-height: 120px; padding: 6px; display: flex; flex-direction: column; justify-content: space-between; cursor: pointer; }
        .cal-day:hover { background: var(--bg-input); }
        .day-num { font-weight: bold; margin-bottom: 4px; font-size: 1rem; color: var(--text-muted); display:flex; justify-content:space-between;}
        .lock-icon { font-size: 0.8rem; }
        .stat-row { display: flex; justify-content: space-between; font-size: 0.75rem; margin-bottom: 2px; }
        .label { color: var(--text-muted); }
        .val-work { color: var(--text-main); }
        .val-std { color: var(--primary); }
        .val-ext { color: var(--success-text); }
        .day-total { border-top: 1px solid var(--border); margin-top: 4px; padding-top: 4px; text-align: right; font-weight: 800; font-size: 0.95rem; color: var(--text-main); }
        .add-btn { display: block; text-align: center; font-size: 2rem; line-height: 1; text-decoration: none; color: var(--border); margin-top: auto; }
        .cal-day:hover .add-btn { color: var(--primary); }
        .summary-card { background: var(--bg-card); padding: 15px; border-radius: 8px; border: 1px solid var(--border); display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 10px; margin-bottom: 20px; }
        .sum-item { text-align: center; }
        .sum-label { font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; }
        .sum-val { font-size: 1.2rem; font-weight: bold; }
        .sum-total-val { font-size: 1.5rem; font-weight: 800; color: var(--primary); }
        
        .dashboard-nav { display: flex; justify-content: space-between; align-items: center; background: var(--bg-card); padding: 10px 15px; border-radius: 8px; border: 1px solid var(--border); margin-bottom: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
        .nav-btn { text-decoration: none; background: var(--bg-input); color: var(--text-main); padding: 8px 16px; border-radius: 6px; border: 1px solid var(--border); font-weight: bold; transition: background 0.2s; }
        .nav-btn:hover { background: var(--border); }
        .nav-title { margin: 0; font-size: 1.2rem; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; }
        ...
        .modal-content { background-color: var(--bg-card); padding: 20px; border-radius: 8px; width: 90%; max-width: 400px; box-shadow: 0 5px 15px rgba(0,0,0,0.3); position: relative; }
        .close-modal { position: absolute; right: 15px; top: 10px; font-size: 1.5rem; cursor: pointer; color: var(--text-muted); }
        .modal-header { border-bottom: 1px solid var(--border); padding-bottom: 10px; margin-bottom: 15px; text-align: center; font-weight: bold; font-size: 1.2rem; }
        .modal-row { display: flex; justify-content: space-between; padding: 5px 0; border-bottom: 1px dashed var(--border); font-size: 0.9rem; }
        .modal-total { display: flex; justify-content: space-between; padding: 10px 0; border-top: 2px solid var(--border); font-weight: bold; font-size: 1.1rem; margin-top: 10px; }
        .modal-nav { display: flex; justify-content: space-between; margin-top: 20px; }
        @media (max-width: 600px) { .cal-header { font-size: 0.7rem; padding: 5px 0; } .cal-day { min-height: 80px; padding: 2px; } .stat-row { font-size: 0.65rem; } }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <div class="dashboard-nav">
        <a href="?m=<?= $prev_m ?>&y=<?= $prev_y ?>" class="nav-btn">&laquo; Prev</a>
        <div class="nav-title"><?= $month_name ?></div>
        <a href="?m=<?= $next_m ?>&y=<?= $next_y ?>" class="nav-btn">Next &raquo;</a>
    </div>

    <div class="summary-card">
        <div class="sum-item"><div class="sum-label">Work</div><div class="sum-val">$<?= number_format($total_work, 2) ?></div></div>
        <div class="sum-item"><div class="sum-label">Std PD</div><div class="sum-val" style="color:var(--primary);">$<?= number_format($total_std_pd, 2) ?></div></div>
        <div class="sum-item"><div class="sum-label">Ext PD</div><div class="sum-val" style="color:var(--success-text);">$<?= number_format($total_ext_pd, 2) ?></div></div>
        <div class="sum-item" style="border-left:1px solid var(--border);"><div class="sum-label">Total</div><div class="sum-total-val">$<?= number_format($grand_total, 2) ?></div></div>
    </div>
    ...
    <div class="cal-wrapper">
        ...
        for ($i = 0; $i < $day_of_week; $i++) { echo "<div class='cal-day' style='opacity:0.5; cursor:default;'></div>"; }

        foreach ($month_data as $date => $d) {
            $lock = $d['is_closed'] ? '<span class="lock-icon">🔒</span>' : '';
            $onclick = $d['is_closed'] ? "openSummary('$date')" : "window.location='index.php?date=$date'";
            
            echo "<div class='cal-day' onclick=\"$onclick\">";
            echo "<div class='day-num'><span>{$d['day_num']}</span>$lock</div>";
            
            if ($d['work'] > 0) echo "<div class='stat-row'><span class='label'>Work</span><span class='val-work'>$" . number_format($d['work'], 0) . "</span></div>";
            if ($d['std_pd'] > 0) echo "<div class='stat-row'><span class='label'>Std</span><span class='val-std'>$" . number_format($d['std_pd'], 0) . "</span></div>";
            if ($d['ext_pd'] > 0) echo "<div class='stat-row'><span class='label'>Ext</span><span class='val-ext'>$" . number_format($d['ext_pd'], 0) . "</span></div>";
