...
if (isset($_POST['toggle_lock'])) {
    try {
        ...
        header("Location: index.php?date=$selected_date" . ($show_entry_mode ? "&view=entry" : "")); exit;
    } catch (Exception $e) { $error = "Lock Error: " . $e->getMessage(); }
}
...
if (isset($_POST['save_truck_log'])) {
    try {
        ...
        $msg = "✅ Daily Log Saved";
    } catch (Exception $e) { $error = "Log Error: " . $e->getMessage(); }
}
...
if (isset($_POST['add_job']) || isset($_POST['save_draft'])) {
    try {
        ...
        if ($is_draft) {
            ...
            header("Location: edit_job.php?id=" . $new_id); exit;
        } else {
            header("Location: index.php?date=" . $_POST['install_date'] . ($show_entry_mode ? "&view=entry" : "")); exit;
        }
...
try {
    ...
    if($prev_res) $last_odo = $prev_res['odometer'];
...
if (!$show_entry_mode) {
    ...
    $mileage_rate = 0.67; $lead_pay_rate = 500.00;
    try { $rows = $db->query("SELECT rate_key, amount FROM rate_card WHERE rate_key IN ('IRS_MILEAGE', 'LEAD_PAY')")->fetchAll(PDO::FETCH_ASSOC); foreach($rows as $r) { if($r['rate_key'] == 'IRS_MILEAGE') $mileage_rate = (float)$r['amount']; if($r['rate_key'] == 'LEAD_PAY') $lead_pay_rate = (float)$r['amount']; } } catch(Exception $e) {}
    $user_map = []; try { $stmt = $db->query("SELECT id, username FROM users"); while($row = $stmt->fetch(PDO::FETCH_ASSOC)) { $user_map[$row['id']] = ucfirst($row['username']); } } catch(Exception $e) {}
    $job_revenue = 0; $active_weeks_by_user = []; $stmt = $db->prepare("SELECT * FROM jobs WHERE install_date BETWEEN ? AND ? ORDER BY install_date DESC"); $stmt->execute([$start_date, $end_date]); $admin_jobs = $stmt->fetchAll(PDO::FETCH_ASSOC);
    foreach ($admin_jobs as $j) { $job_revenue += $j['pay_amount']; if ($j['install_type'] !== 'DO' && $j['install_type'] !== 'ND') { $ts = strtotime($j['install_date']); $week_end = date('Y-m-d', strtotime($j['install_date'] . " +" . (6 - date('w', $ts)) . " days")); $active_weeks_by_user[$j['user_id']][$week_end] = true; } }
    $lead_pay_total = 0; foreach ($active_weeks_by_user as $uid => $weeks) { $lead_pay_total += (count($weeks) * $lead_pay_rate); } $gross_revenue = $job_revenue + $lead_pay_total;
    $total_miles = 0; $total_fuel_cost = 0; try { $stmt = $db->prepare("SELECT mileage, fuel_cost FROM daily_logs WHERE log_date BETWEEN ? AND ?"); $stmt->execute([$start_date, $end_date]); while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { $total_miles += $row['mileage']; $total_fuel_cost += $row['fuel_cost']; } } catch (Exception $e) {}
    $mileage_deduction = $total_miles * $mileage_rate; $net_income = $gross_revenue - $mileage_deduction;
}
// =========================================================
//  VIEW 2: JOB ENTRY
// =========================================================
    ...
    try { 
        ...
        foreach ($daily_jobs as $j) { 
            ...
            if ($j['install_type'] !== 'DO' && $j['install_type'] !== 'ND') $has_work_today = true;
        }
        
        $is_sunday = (date('w', strtotime($selected_date)) == 0);
        $day_pd = 0;
        if ($is_sunday || $has_work_today) $day_pd += $rate_std;
        ...
        if ($day_log_data && $day_log_data['extra_per_diem'] == 1) $day_pd += $rate_ext;
        
        $daily_total = $day_job_pay + $day_pd;

        $ts = strtotime($selected_date);
        if (date('N', $ts) == 1) { $start_of_week = $selected_date; } else { $start_of_week = date('Y-m-d', strtotime('last monday', $ts)); }
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $d_pay = 0; $d_work = false;
            foreach ($week_jobs_all as $wj) {
                if ($wj['install_date'] === $loop_date) {
                    $d_pay += $wj['pay_amount'];
                    if ($wj['install_type'] !== 'DO' && $wj['install_type'] !== 'ND') $d_work = true;
                }
            }
            $d_pd_calc = 0;
            if ($is_sun || $d_work) $d_pd_calc += $rate_std;
            if (isset($week_logs_all[$loop_date]) && $week_logs_all[$loop_date] == 1) $d_pd_calc += $rate_ext;
            $weekly_grand_total += ($d_pay + $d_pd_calc);
        }
        $lead_pay_amt = 0; 
        if (function_exists('get_lead_pay_amount')) { $lead_pay_amt = get_lead_pay_amount($db); } 
        $has_billable = false; 
        if (function_exists('has_billable_work')) { $has_billable = has_billable_work($db, $user_id, $start_of_week, $end_of_week); } 
        if ($has_billable && $lead_pay_amt > 0) $weekly_grand_total += $lead_pay_amt;
...
<head>
    ...
    <style>
        .grow-wrap { width: 100%; box-sizing: border-box; }
        .grow-wrap textarea {
            width: 100%; overflow: hidden; resize: none; min-height: 40px;
            padding: 10px; border: 1px solid var(--border); background: var(--bg-input);
            color: var(--text-main); border-radius: 6px; font-family: inherit; font-size: 1rem;
            line-height: 1.4; transition: height 0.1s ease;
        }
        .grow-wrap textarea:focus { border-color: var(--primary); outline: none; }
...
<?php if (!$show_entry_mode): ?>
    <div style="margin-bottom:20px; display:flex; justify-content:space-between; align-items:end;">
        <div><h2 style="margin:0;">📅 Month to Date</h2><div style="color:var(--text-muted); font-size:0.9rem;"><?= date('F 1') ?> - <?= date('F j, Y') ?></div></div>
        <div style="text-align:right;"><a href="?view=entry" class="btn" style="padding:6px 12px;">📝 Manually Enter Job</a> <a href="financials.php" class="btn" style="padding:6px 12px;">Full Year &rarr;</a></div>
    </div>
    <div class="kpi-grid">
        <div class="kpi-card"><div class="kpi-label">Gross Revenue</div><div class="kpi-value positive">$<?= number_format($gross_revenue, 2) ?></div></div>
        <div class="kpi-card"><div class="kpi-label">Mileage Deduction</div><div class="kpi-value">$<?= number_format($mileage_deduction, 2) ?></div></div>
        <div class="kpi-card"><div class="kpi-label">Est. Net Income</div><div class="kpi-value" style="color:var(--primary);">$<?= number_format($net_income, 2) ?></div></div>
        <div class="kpi-card"><div class="kpi-label">Actual Fuel</div><div class="kpi-value negative">$<?= number_format($total_fuel_cost, 2) ?></div></div>
    </div>
    <div class="box">
        <h3 style="margin-top:0;">📋 Recent Activity (All Users)</h3>
        <table style="width:100%; border-collapse:collapse; font-size:0.9rem;">
            <tr style="text-align:left; color:var(--text-muted); border-bottom:2px solid var(--border);"><th style="padding:8px;">Date</th><th>Tech</th><th>Ticket</th><th style="text-align:right;">Pay</th><th></th></tr>
            <?php $recent = array_slice($admin_jobs, 0, 8); foreach($recent as $j): $tech_name = $user_map[$j['user_id']] ?? 'Unknown'; ?>
            <tr style="border-bottom:1px solid var(--border);">
                <td style="padding:10px 8px;"><?= date('m/d', strtotime($j['install_date'])) ?></td>
                <td style="padding:10px 8px;"><span class="tech-badge"><?= htmlspecialchars($tech_name) ?></span></td>
                <td onclick="window.location='edit_job.php?id=<?=$j['id']?>'" style="padding:10px 8px; cursor:pointer; font-weight:bold; color:var(--primary);"><?= $j['ticket_number'] ?></td>
                <td style="padding:10px 8px; text-align:right; font-weight:bold;">$<?= number_format($j['pay_amount'], 2) ?></td>
                <td style="padding:10px 8px; text-align:right;">
                    <a href="?delete=<?=$j['id']?>&date=<?=$selected_date?>" onclick="if(!confirm('Delete Job?')) return false;" class="btn btn-small btn-danger">🗑️</a>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
...
<?php else: ?>
    <?php if ($is_admin): ?><div style="margin-bottom:15px;"><a href="index.php" class="btn">&larr; Back to Dashboard</a></div><?php endif; ?>
    <?php if($msg): ?><div class="alert" style="border-left:4px solid var(--success-text);"><?=$msg?></div><?php endif; ?>
    <?php if($error): ?><div class="alert" style="background:var(--danger-bg); color:var(--danger-text);"><?=$error?></div><?php endif; ?>

    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
        <a href="?date=<?= date('Y-m-d', strtotime($selected_date . ' -1 day')) ?>&view=entry" class="btn btn-small btn-secondary">◀</a>
        <h3 style="margin:0;"><?= date('D, M j', strtotime($selected_date)) ?></h3>
        <a href="?date=<?= date('Y-m-d', strtotime($selected_date . ' +1 day')) ?>&view=entry" class="btn btn-small btn-secondary">▶</a>
    </div>

    <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:10px;">
        <div class="box" style="text-align:center; padding:10px;"><div style="font-size:0.8rem; color:var(--text-muted);">Day Total</div><div style="font-size:1.5rem; font-weight:bold; color:var(--primary);">$<?= number_format($daily_total, 2) ?></div></div>
        <div class="box" style="text-align:center; padding:10px;"><div style="font-size:0.8rem; color:var(--text-muted);">Week Total</div><div style="font-size:1.5rem; font-weight:bold; color:var(--success-text);">$<?= number_format($weekly_grand_total, 2) ?></div></div>
    </div>

    <h4 style="margin:0 0 10px; color:var(--text-muted);">Jobs Today</h4>
    <?php if(empty($daily_jobs)): ?><div class="box" style="text-align:center; padding:20px; color:var(--text-muted); margin-bottom:20px;">No jobs entered for today.</div>
    <?php else: foreach($daily_jobs as $job): ?>
        <div class="box" style="margin-bottom:10px; display:flex; justify-content:space-between; align-items:center;">
            <div onclick="window.location='edit_job.php?id=<?=$job['id']?>'" style="cursor:pointer; flex:1;">
                <div style="font-weight:bold;"><?=$job['ticket_number']?></div><div style="font-size:0.85rem; color:var(--text-muted);"><?=$job['install_type']?> • <?=$job['cust_city']?></div>
            </div>
            <div style="text-align:right;">
                <div style="font-weight:bold; color:var(--success-text); margin-bottom:5px;">$<?= number_format($job['pay_amount'], 2) ?></div>
                <a href="?date=<?=$selected_date?>&delete=<?=$job['id']?>&view=entry" onclick="if(!confirm('Delete this job?')) return false;" class="btn btn-small btn-danger">🗑️ Delete</a>
            </div>
        </div>
    ...
    <div class="box" style="margin-bottom:20px; background:var(--bg-card); border-left:4px solid var(--primary);">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
            ...
            <form method="post" style="margin:0;"><?php if ($is_day_locked): ?>
                <button type="submit" name="toggle_lock" value="unlock" class="btn btn-small btn-secondary">🔓 Unlock</button>
            <?php else: ?>
                <button type="submit" name="toggle_lock" value="lock" class="btn btn-small btn-danger">🔒 Lock</button>
            <?php endif; ?></form>
        </div>
        ...
        <div id="truckLogContent" style="display:none;">
            <?php if ($is_day_locked): ?>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; text-align:left;">
                    <div><div style="font-size:0.75rem; color:var(--text-muted);">Odometer</div><div style="font-weight:bold;"><?= $day_log['odometer'] ?></div></div>
                    <div><div style="font-size:0.75rem; color:var(--text-muted);">Miles</div><div style="font-weight:bold;"><?= $day_log['mileage'] ?></div></div>
                    <div><div style="font-size:0.75rem; color:var(--text-muted);">Gallons</div><div style="font-weight:bold;"><?= $day_log['gallons'] ?></div></div>
                    <div><div style="font-size:0.75rem; color:var(--text-muted);">Fuel Cost</div><div style="font-weight:bold;">$<?= $day_log['fuel_cost'] ?></div></div>
                    <div style="grid-column: span 2; border-top:1px solid var(--border); padding-top:10px; display:flex; justify-content:space-between; align-items:center;">
                        <div><div style="font-size:0.75rem; color:var(--text-muted);">Performance</div><div style="font-weight:bold; color:var(--success-text);"><?= $l_mpg ?></div></div>
                        <?php if($day_log['extra_per_diem']): ?><span style="background:var(--success-text); color:#fff; padding:2px 6px; border-radius:4px; font-size:0.7rem;">EXTRA PD</span><?php endif; ?>
                    </div>
                </div>
                ...
                <form method="post" style="display:flex; flex-direction:column; gap:10px;">
                    <div style="display:flex; gap:10px;">
                        <div style="flex:1;">
                            ...
                            <input type="number" name="odometer" value="<?=htmlspecialchars($day_log['odometer'])?>" data-prev-odo="<?=$last_odo?>" oninput="updateMiles()" style="padding:6px; width:100%;">
                        </div>
                        <div style="flex:1;">
                            <label style="font-size:0.75rem;">Miles</label>
                            <input type="number" id="truck_miles" name="mileage" value="<?=htmlspecialchars($day_log['mileage'])?>" oninput="calculateMPG()" style="padding:6px; width:100%;">
                        </div>
                    </div>
                    ...
                    <div style="display:flex; gap:10px;">
                        <div style="flex:1;">
                            ...
                            <input type="number" step="0.001" id="truck_gallons" name="gallons" value="<?=htmlspecialchars($day_log['gallons'])?>" oninput="calculateMPG()" style="padding:6px; width:100%;">
                        </div>
                        <div style="flex:1;">
                            <label style="font-size:0.75rem;">Fuel $</label>
                            <input type="number" step="0.01" name="fuel_cost" value="<?=htmlspecialchars($day_log['fuel_cost'])?>" style="padding:6px; width:100%;">
                        </div>
                    </div>
                    ...
                    <div style="display:flex; gap:10px; align-items:end;">
                        <div style="flex:1;">
                            ...
                            <div id="mpg_wrapper" style="border:1px solid var(--border); border-radius:4px; padding:6px; background:var(--bg-input); text-align:center;">
                                <span id="mpg_result" style="font-weight:bold; color:var(--text-muted);">--.-- MPG</span>
                            </div>
                        </div>
        ...
        <div class="box">
            <form method="post">
                <div class="grid-container">
                    <div><label>Date</label><input type="date" name="install_date" value="<?=$selected_date?>" required></div>
                    <div><label>Ticket #</label><input type="text" name="ticket_number" required></div>
                </div>
                <div class="full-width" style="margin-top:10px;">
                    <label>Job Code</label>
                    <select name="install_type" id="install_type" onchange="toggleFields()" required style="width:100%; padding:10px;">
                        <option value="" disabled selected>Select Code...</option>
                        <?php if(!empty($job_codes_list)): foreach($job_codes_list as $jc): ?><option value="<?= htmlspecialchars($jc['rate_key']) ?>"><?= htmlspecialchars($jc['rate_key']) ?> - <?= htmlspecialchars($jc['description']) ?></option><?php endforeach; else: ?><option value="I-6600">I-6600 - Install</option><option value="R-6600">R-6600 - Repair</option><option value="TC">TC - Trouble Call</option><option value="F008">F008 - Wire Run</option><option value="F009">F009 - Jack Install</option><option value="F011">F011 - Misc</option><option value="DO">DO - Day Off</option><?php endif; ?>
                    </select>
                </div>
                ...
                <div id="secDetails" style="margin-top:15px;">
                    ...
                    <div id="secCustomer">
                        <div class="grid-container"><div><input type="text" name="cust_fname" placeholder="First Name"></div><div><input type="text" name="cust_lname" placeholder="Last Name"></div></div>
                        <div style="margin-top:10px;"><input type="text" name="cust_street" placeholder="Address" style="width:100%;"></div>
                        <div class="grid-container" style="margin-top:10px;"><div><input type="text" name="cust_city" placeholder="City"></div><div><input type="text" name="cust_zip" placeholder="Zip"></div></div>
                        <div class="grid-container" style="margin-top:10px;"><div><input type="text" name="cust_state" placeholder="State"></div><div><input type="text" name="cust_phone" placeholder="Phone"></div></div>
                        <hr style="margin:15px 0; border:0; border-top:1px solid var(--border);">
                    </div>
                    ...
                    <div id="groupMissed" style="display:none; background:var(--bg-input); padding:15px; border-radius:8px; margin-bottom:15px;">
                        ...
                        <div class="grow-wrap spacer"><textarea name="why_missed" placeholder="Why Missed?"></textarea></div>
                        <div class="grow-wrap spacer"><textarea name="supervisor" placeholder="Supervisor Contacted"></textarea></div>
                    ...
                    <div id="groupRepair" style="display:none; background:var(--bg-input); padding:15px; border-radius:8px; margin-bottom:15px;">
                        ...
                        <div class="grow-wrap spacer"><textarea name="complaint" placeholder="Customer Complaint"></textarea></div>
                        <div class="grow-wrap spacer"><textarea name="resolution" placeholder="Resolution Steps"></textarea></div>
                        <div class="grow-wrap spacer"><textarea name="equip_replaced" placeholder="Equipment Replaced"></textarea></div>
                        <div class="grow-wrap"><textarea name="service_restored" placeholder="Service Restored?"></textarea></div>
                    </div>
                    ...
                    <div id="groupTechStandard">
                        <div id="subTechSpecs">
                            ...
                            <div class="grid-container spacer">
                                <div><input type="number" step="0.01" name="tici_hub" placeholder="Light @ Hub (e.g. -13.50)" onchange="forceNegative(this)"></div>
                                <div><input type="number" step="0.01" name="tici_ont" placeholder="Light @ ONT (e.g. -16.20)" onchange="forceNegative(this)"></div>
                            </div>
                            <div class="grid-container spacer">
                    ...
                    <div style="margin-top:10px;" id="groupNotes">
                        <div style="margin-bottom:10px;">
                            ...
                            <div class="grow-wrap"><textarea id="misc_notes" name="misc_notes" placeholder="Dog in yard, moved couch, etc..."></textarea></div>
                        </div>
                        <div style="margin-bottom:5px; text-align:right;">
                            <button type="button" onclick="copyNotes()" class="btn btn-small btn-secondary">Copy Notes</button>
                        </div>
                        <div class="grow-wrap"><textarea name="addtl_work" placeholder="Full Notes Preview..." readonly style="background:#f3f4f6; color:#555;"></textarea></div>
                    </div>
                </div>
                
                <div style="display:flex; gap:10px; margin-top:20px;">
                    <button type="submit" name="save_draft" class="btn btn-secondary" style="flex:1;">💾 Save Draft</button>
