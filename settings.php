...
<head>
    ...
    <style>
        ...
        .rate-grid:last-child { border-bottom: none; }
        
        .rate-key { font-weight: bold; color: var(--primary); font-family: monospace; font-size: 1.05rem; }
        .rate-desc { font-size: 0.9rem; color: var(--text-main); line-height: 1.3; }
        .rate-input input { 
            width: 100%; padding: 8px; text-align: right; border: 1px solid var(--border); 
            border-radius: 4px; background: var(--bg-input); color: var(--text-main); font-weight: bold;
        }

        .header-row {
            background: var(--bg-input);
            font-weight: bold; color: var(--text-muted); font-size: 0.85rem; text-transform: uppercase;
        ...
        .backup-section { margin-top: 30px; padding: 20px; border: 1px dashed var(--border); text-align: center; border-radius: 8px; background: var(--bg-card); }
    </style>
</head>
<body>
...
<div class="container">
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
        ...
        <a href="index.php" class="btn" style="background:var(--bg-input); color:var(--text-main); border:1px solid var(--border);">Back</a>
    </div>
        ...
        <form method="post">
            ...
            <div class="rate-grid">
                ...
                <div class="rate-input">
                    <input type="text" inputmode="decimal" name="rates[<?=$r['rate_key']?>]" value="<?= number_format($r['amount'], (strpos($r['rate_key'],'TAX')!==false ? 2 : 2)) ?>">
                </div>
            </div>
            <?php endforeach; ?>
    ...
    <div class="backup-section">
        ...
        <p style="color:var(--text-muted); font-size:0.9rem;">Manage database snapshots to keep your records safe.</p>
        <a href="backup.php" class="btn" style="background:#000; color:#fff; text-decoration:none; display:inline-block; padding:10px 20px;">Open Backup Manager</a>
    </div>
